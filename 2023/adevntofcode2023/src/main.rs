...
fn findPositionOfStringInString(string: &str, substring: &str) -> i8 {
    let mut position = -1;
...
fn main() {
    ...
    let mut res = 0;
    for line in input {
        let (mut firstChar, mut lastChar): (char, char) = ('0', '0');
        ...
        for tuple in NUMBER_MAP.iter() {
            let index = findPositionOfStringInString(line, tuple.0);
            if index > -1 && usize::from(index) < line.find(firstChar).unwrap() {
                firstChar = char::from_u32(tuple.1).unwrap();
                println!("{}", lastChar);
                break;
            }
        }
        ...
        let joinedString = (firstChar.to_string()+&lastChar.to_string()).parse::<i32>().unwrap();
        res += joinedString;
